package apijava.apirest.security;

import apijava.apirest.service.DetalheUsuarioServiceImpl;
...
public class JWTConfiguracao extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.csrf().disable().authorizeRequests().antMatchers(HttpMethod.POST, "/login").permitAll()
                ...
                .addFilter(new JWTValidarFilter(authenticationManager())).sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS);
    }
